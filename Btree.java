...
//    public void traverseAndUpdateBitmapFromFCB(FCB fcb) {
//        String startBlock = fcb.getDataStartBlock();
//        int startNodeIndex = Integer.parseInt(startBlock);
//
//        if (startNodeIndex < 0 || nodes[startNodeIndex] == null)
//            return;
//
//        Queue<Integer> queue = new LinkedList<>();
...
//
//                // Update the bitmap for the current node index
//                updateBitmap(currentNodeIndex);
//
//                // Add child nodes of the current node to the queue for later processing
//                for (int j = 0; j <= currentNode.values.length; j++) { // Assuming children length equals values length + 1
//                    if (j < currentNode.children.length && currentNode.children[j] != -1) {
