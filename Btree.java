...
final class Btree {
    ...
    public void Display(int nodeId) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < levelLength; i++) {
                ...
                int count = 0;
                for (KeyPointer val : currentNode.values) {
                    if (val != null && val.getKey() != -1) {
                        if (count > 0) {
                            System.out.print(",");
                        }
                ...
                for (int j = 0; j <= NODESIZE; j++) { // Iterate through all possible children
                    int childId = currentNode.children[j].getKey();
                    if (childId != -1) {
                        queue.add(childId);
                    } else {
                        break;
    ...
    private boolean nodeLookup(int value, int pointer, String s) {
        ...
        if (isLeaf(node)) {
            ...
            return nodeLookup(value, node.children[i].getKey(), s);
        }
    }
    ...
    private void splitChild(int parent, int i, int fullChild) {
        ...
        // Move the children pointers to the right
        // build a new KeyPointer object with the newChild and ""
        nodes[parent].children[i + 1] = new KeyPointer(newChild, "9999999");
...
//        Arrays.fill(child.values, promoteIndex, NODESIZE, -1);
        // Clear the values that were moved to the new node in the original child node
        for (int j = promoteIndex; j < NODESIZE; j++) {
            child.values[j] = null; // Assuming null indicates an empty slot
        }
    ...
    private int insertNonFull(int nodeIndex, int value) {
        ...
        if (isLeaf(node)) {
            ...
            while (i >= 0 && value < node.values[i].getKey()) {
                node.values[i + 1] = node.values[i];
                i--;
            }
            node.values[i + 1] = new KeyPointer(value, "9999000");
//            node.values[i + 1] = value;
        ...
        if (nodes[node.children[i].getKey()].size == NODESIZE) {
            splitChild(nodeIndex, i, node.children[i].getKey());
            if (value > node.values[i].getKey()) {
                i++;
            }
        }
        // Recurse into the appropriate child node for insertion
        return insertNonFull(node.children[i].getKey(), value);
    }
    ...
    private int nodeInsert(int value, int pointer) {
        ...
        if (currNode.size == NODESIZE) { // If current node is full
            if (pointer == root) { // if current node is root
                ...
                // build a new KeyPointer object with the newRoot and ""
                nodes[newRoot].children[0] = new KeyPointer(pointer, "9999999");
...
//    node.children =  new int[NODESIZE + 1];
        ...
        node.children = new KeyPointer[NODESIZE + 1];
        ...
        for (int i = 0; i < NODESIZE + 1; i++) {
            node.children[i] = new KeyPointer(); // Using default constructor
        }
